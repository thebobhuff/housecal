...
function App() {
  ...
  const [settings, setSettings] = useState({ scene_duration_seconds: 12, night_start_hour: 20, night_end_hour: 7, scene_enabled: defaultSceneEnabled });
  ...
  return <div className={`app-shell ${scene === 1 ? 'photo-display-active' : ''}`}>
    ...
    <main className={`scene-main scene-${scene}`}>
      {scene === 0 && <CalendarScene view={view} setView={setView} week={currentWeek} todayLabel={formatToday()} family={family} activeFilter={activeFilter} setActiveFilter={setActiveFilter} filteredEvents={filteredEvents} events={events} setShowModal={setShowModal} setShowMealModal={setShowMealModal} setToast={setToast} openPhotosPicker={openPhotosPicker} done={done} routines={routines} completeChore={completeChore} editable={Boolean(access.session)}/>}
    ...
    {showModal && <AddEventModal onClose={() => setShowModal(false)} onAdd={addEvent}/>} {showMealModal && <MealModal householdId={access.household?.id} meal={mealPlan} onClose={() => setShowMealModal(false)} onSaved={(meal) => { setMealPlan(meal); setShowMealModal(false); setToast('Meal plan saved'); }}/>} {showSettings && <SettingsModal householdId={access.household?.id} settings={settings} onClose={() => setShowSettings(false)} onSaved={(next) => { setSettings(next); setShowSettings(false); setToast('Display settings saved'); }}/>} {showRoutineManager && <RoutineManagerModal householdId={access.household?.id} routines={routines} onClose={() => setShowRoutineManager(false)} onChanged={setRoutines}/>} {pairingCode && <PairingModal code={pairingCode} onClose={() => setPairingCode('')}/>} {toast && <div className="toast">{toast}{photosPickerUrl && <a href={photosPickerUrl} target="_blank" rel="noreferrer" onClick={() => setPhotosPickerUrl('')}>Open Google Photos</a>}</div>}
  </div>;
}

function CalendarScene({ view, setView, week, todayLabel, family, activeFilter, setActiveFilter, filteredEvents, events, setShowModal, setShowMealModal, setToast, openPhotosPicker, done, routines, completeChore, editable }) { return <div className="scene-content calendar-scene">
...
function SettingsModal({ householdId, settings, onClose, onSaved }) { const [duration, setDuration] = useState(settings.scene_duration_seconds || 12); const [nightStart, setNightStart] = useState(settings.night_start_hour ?? 20); const [nightEnd, setNightEnd] = useState(settings.night_end_hour ?? 7); const [enabled, setEnabled] = useState({ ...defaultSceneEnabled, ...(settings.scene_enabled || {}) }); const save = async () => { try { const result = await saveHouseholdSettings(householdId, { scene_duration_seconds: Number(duration), night_start_hour: Number(nightStart), night_end_hour: Number(nightEnd), scene_enabled: enabled }); onSaved(result); } catch { /* Keep the modal open when the settings write fails. */ } }; return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="modal"><div className="modal-heading"><div><p className="eyebrow">DISPLAY SETTINGS</p><h2>Make it feel like home.</h2></div><button className="close-button" onClick={onClose}><X size={20}/></button></div><p className="settings-section-label">PLAYLIST SCENES</p><div className="scene-settings-grid">{scenes.map((item) => <label className="scene-setting" key={item.label}><input type="checkbox" checked={enabled[item.label] !== false} onChange={(event) => setEnabled((current) => ({ ...current, [item.label]: event.target.checked }))}/><span>{item.label}</span></label>)}</div><label className="modal-field-gap">Scene duration (seconds)<input type="number" min="5" max="180" value={duration} onChange={(event) => setDuration(event.target.value)}/></label><div className="form-row"><label>Night starts<input type="number" min="0" max="23" value={nightStart} onChange={(event) => setNightStart(event.target.value)}/></label><label>Night ends<input type="number" min="0" max="23" value={nightEnd} onChange={(event) => setNightEnd(event.target.value)}/></label></div><button className="save-button" disabled={!householdId || !Object.values(enabled).some(Boolean)} onClick={save}>Save display settings <Check size={17}/></button></div></div> }
