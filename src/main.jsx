...
function PairingModal({ code, onClose }) { return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="modal pairing-modal"><div className="modal-heading"><div><p className="eyebrow">FIRE TV PAIRING</p><h2>Enter this code on the display.</h2></div><button className="close-button" onClick={onClose}><X size={20}/></button></div><div className="pairing-code">{code}</div><p className="pairing-note">This code expires in 10 minutes and can only be used once.</p><button className="save-button" onClick={onClose}>Done <Check size={17}/></button></div></div> }
