...
import { createDisplayPairing, createHousehold, createRoutine, deleteRoutine, ensureHousecalDefaults, getCurrentSession, loadHousecalState, loadLocalNews, loadTraffic, pollGooglePhotosPicker, saveHouseholdName, saveHouseholdSettings, saveMealPlan, setRoutineCompletion, signOut, startGoogleConnection, startGooglePhotosPicker, supabase, syncGoogleCalendar, validateDisplaySession } from './lib/supabase';
...
function CalendarScene({ view, setView, week, todayLabel, familyName, family, activeFilter, setActiveFilter, filteredEvents, events, photos, setShowModal, setShowMealModal, setToast, openPhotosPicker, done, routines, completeChore, editable }) { const albumTitle = photos.length ? `SELECTED PHOTOS · ${photos.length}` : 'FAMILY PHOTOS'; return <div className="scene-content calendar-scene">
...
function SettingsModal({ householdId, householdName, settings, onClose, onSaved }) { const [name, setName] = useState(householdName || 'Our family'); const [duration, setDuration] = useState(settings.scene_duration_seconds || 12); const [nightStart, setNightStart] = useState(settings.night_start_hour ?? 20); const [nightEnd, setNightEnd] = useState(settings.night_end_hour ?? 7); const [enabled, setEnabled] = useState({ ...defaultSceneEnabled, ...(settings.scene_enabled || {}) }); const save = async () => { try { const savedName = await saveHouseholdName(householdId, name.trim()); const result = await saveHouseholdSettings(householdId, { scene_duration_seconds: Number(duration), night_start_hour: Number(nightStart), night_end_hour: Number(nightEnd), scene_enabled: enabled }); onSaved(result, savedName); } catch { /* Keep the modal open when the settings write fails. */ } }; return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="modal"><div className="modal-heading"><div><p className="eyebrow">FAMILY SETTINGS</p><h2>Make it feel like home.</h2></div><button className="close-button" onClick={onClose}><X size={20}/></button></div><label>Family name<input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. The Huffs"/></label><p className="settings-section-label modal-field-gap">PLAYLIST SCENES</p><div className="scene-settings-grid">{scenes.map((item) => <label className="scene-setting" key={item.label}><input type="checkbox" checked={enabled[item.label] !== false} onChange={(event) => setEnabled((current) => ({ ...current, [item.label]: event.target.checked }))}/><span>{item.label}</span></label>)}</div><label className="modal-field-gap">Scene duration (seconds)<input type="number" min="5" max="180" value={duration} onChange={(event) => setDuration(event.target.value)}/></label><div className="form-row"><label>Night starts<input type="number" min="0" max="23" value={nightStart} onChange={(event) => setNightStart(event.target.value)}/></label><label>Night ends<input type="number" min="0" max="23" value={nightEnd} onChange={(event) => setNightEnd(event.target.value)}/></label></div><button className="save-button" disabled={!householdId || !name.trim() || !Object.values(enabled).some(Boolean)} onClick={save}>Save family settings <Check size={17}/></button></div></div> }
