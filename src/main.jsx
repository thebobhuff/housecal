...
function App() {
  ...
  return <div className={`app-shell ${scene === 1 ? 'photo-display-active' : ''}`}>
    ...
    <main className={`scene-main scene-${scene}`}>
      {scene === 0 && <CalendarScene view={view} setView={setView} week={currentWeek} todayLabel={formatToday()} family={family} activeFilter={activeFilter} setActiveFilter={setActiveFilter} filteredEvents={filteredEvents} events={events} photos={photos} setShowModal={setShowModal} setShowMealModal={setShowMealModal} setToast={setToast} openPhotosPicker={openPhotosPicker} done={done} routines={routines} completeChore={completeChore} editable={Boolean(access.session)}/>}
...
function CalendarScene({ view, setView, week, todayLabel, family, activeFilter, setActiveFilter, filteredEvents, events, photos, setShowModal, setShowMealModal, setToast, openPhotosPicker, done, routines, completeChore, editable }) { const albumTitle = photos.length ? `SELECTED PHOTOS · ${photos.length}` : 'FAMILY PHOTOS'; return <div className="scene-content calendar-scene">
