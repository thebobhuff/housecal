...
import { createDisplayPairing, createHousehold, createRoutine, deleteRoutine, ensureHousecalDefaults, getCurrentSession, loadHousecalState, loadLocalNews, loadTraffic, pollGooglePhotosPicker, replaceHouseholdPeople, resolveLocation, saveHouseholdName, saveHouseholdSettings, saveMealPlan, setRoutineCompletion, signOut, startGoogleConnection, startGooglePhotosPicker, supabase, syncGoogleCalendar, validateDisplaySession } from './lib/supabase';
...
function App() {
  ...
  const addEvent = (event) => { setEvents((current) => [...current, { ...event, id: Date.now(), color: people.find((person) => person.name === event.person)?.color || '#6d7b70' }]); setShowModal(false); setToast('Added to the family calendar'); setTimeout(() => setToast(''), 2200); };
  ...
  return <div className={`app-shell ${scene === 1 ? 'photo-display-active' : ''}`} style={{ '--weather-photo': photos[0] ? `url('${photos[0]}')` : 'none' }}>
    ...
    <main className={`scene-main scene-${scene}`}>
      {scene === 0 && <CalendarScene
        todayLabel={formatToday()} familyName={familyName} people={people} activeFilter={activeFilter} setActiveFilter={setActiveFilter} filteredEvents={filteredEvents} photos={photos} setShowModal={setShowModal} setToast={setToast} openPhotosPicker={openPhotosPicker} done={done} routines={routines} completeChore={completeChore} editable={Boolean(access.session)} currentTime={currentTime} weather={weather} location={weatherLocation} onManagePeople={() => setShowFamilyManager(true)}/>}
      ...
      {scene === 3 && <WeatherScene weather={weather} location={weatherLocation} photos={photos} setToast={setToast}/>}
...
function CalendarScene({ todayLabel, familyName, people, activeFilter, setActiveFilter, filteredEvents, photos, setShowModal, setToast, openPhotosPicker, done, routines, completeChore, editable, currentTime, weather, location, onManagePeople }) {
  ...
  return <div className="scene-content calendar-scene">
    ...
    <div className="family-filters today-people-filters">{people.map((person) => <button key={person.id || person.name} className={activeFilter === person.name ? 'active' : ''} onClick={() => setActiveFilter(person.name)}><span style={{ background: person.color }}></span>{person.name}</button>)}<button className="manage-family" onClick={onManagePeople}><Settings2 size={16}/> Edit people</button></div>
...
function WeatherScene({ weather, location, photos = [], setToast }) {
  const current = weather.data;
...
function FamilyManagerModal({ householdId, people, onClose, onSaved }) { const [draft, setDraft] = useState(people.map((person) => ({ ...person }))); const [error, setError] = useState(''); const update = (index, changes) => setDraft((current) => current.map((person, personIndex) => personIndex === index ? { ...person, ...changes } : person)); const save = async () => { try { if (!draft.some((person) => person.name.trim() && person.show_on_display !== false)) throw new Error('Keep at least one person enabled'); const saved = await replaceHouseholdPeople(householdId, draft); onSaved(saved); } catch (saveError) { setError(saveError.message || 'Unable to save family members'); } }; return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="modal family-manager-modal"><div className="modal-heading"><div><p className="eyebrow">FAMILY MEMBERS</p><h2>Who appears on HouseCal?</h2></div><button className="close-button" onClick={onClose}><X size={20}/></button></div><p className="pairing-note">These are the people available in calendar filters. Google Calendar remains connected at the household level.</p><div className="family-manager-list">{draft.map((person, index) => <div className="family-manager-row" key={person.id || index}><input value={person.name} onChange={(event) => update(index, { name: event.target.value })} placeholder="Person name"/><label><input type="checkbox" checked={person.show_on_display !== false} onChange={(event) => update(index, { show_on_display: event.target.checked })}/> Show</label></div>)}</div><button className="text-action" onClick={() => setDraft((current) => [...current, { name: '', color: '#6d7b70', tint: '#dfe8df', show_on_display: true }])}><Plus size={15}/> Add person</button>{error && <p className="settings-error">{error}</p>}<button className="save-button" disabled={!householdId} onClick={save}>Save family members <Check size={17}/></button></div></div> }
