import { googleConfig, exchangeCode, googleUserInfo, encrypt } from '../_shared/google.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: connectionError } = await supabase.from('google_connections').upsert({ household_id: oauthState.household_id, user_id: oauthState.user_id, provider: oauthState.provider, access_token_encrypted: await encrypt(tokens.access_token), refresh_token_encrypted: await encrypt(tokens.refresh_token), expires_at: new Date(Date.now() + (tokens.expires_in || 3600) * 1000).toISOString(), scope: tokens.scope, profile_name: profile.name || null, profile_email: profile.email || null, profile_picture_url: profile.picture || null, updated_at: new Date().toISOString() }, { onConflict: 'household_id,provider' });
